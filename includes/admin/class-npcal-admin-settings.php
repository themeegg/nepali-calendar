<?php
/**
 * Nepali Calendar Admin Settings Class
 *
 * @author   ThemeEgg
 * @category Admin
...
class NPCAL_Admin_Settings {
	...
	public static function save() {
		...
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'nepali-calendar-settings' ) ) {
			die( __( 'Action failed. Please refresh the page and retry.', 'nepali-calendar' ) );
		}

		// Trigger actions
		do_action( 'nepali_calendar_settings_save_' . $current_tab );
		do_action( 'nepali_calendar_update_options_' . $current_tab );
		do_action( 'nepali_calendar_update_options' );

		self::add_message( __( 'Your settings have been saved.', 'nepali-calendar' ) );
...
*@param string $text
	...
	public static function output() {
		...
		$tabs = apply_filters( 'nepali_calendar_settings_tabs_array', array() );
		...
		foreach ( $options as $value ) {
			...
			switch ( $value['type'] ) {
				...
				case 'title':
					...
					if ( ! empty( $value['id'] ) ) {
						do_action( 'nepali_calendar_settings_' . sanitize_title( $value['id'] ) );
					}
					break;
				...
				case 'sectionend':
					if ( ! empty( $value['id'] ) ) {
						do_action( 'nepali_calendar_settings_' . sanitize_title( $value['id'] ) . '_end' );
					}
					echo '</table>';
					if ( ! empty( $value['id'] ) ) {
						do_action( 'nepali_calendar_settings_' . sanitize_title( $value['id'] ) . '_after' );
					}
					break;
				...
				case 'image_width' :
					...
					if ( has_filter( 'nepali_calendar_get_image_size_' . $image_size ) ) {
						$disabled_attr = 'disabled="disabled"';
						$disabled_message = "<p><small>" . __( 'The settings of this image size have been disabled because its values are being overwritten by a filter.', 'nepali-calendar' ) . "</small></p>";
					}
					...
					?><tr valign="top">
						...
						<td class="forminp image_width_settings">
							...
							<label><input name="<?php echo esc_attr( $value['id'] ); ?>[crop]" <?php echo $disabled_attr; ?> id="<?php echo esc_attr( $value['id'] ); ?>-crop" type="checkbox" value="1" <?php checked( 1, $crop ); ?> /> <?php _e( 'Hard crop?', 'nepali-calendar' ); ?></label>
				...
				case 'single_select_page' :
					...
					?><tr valign="top" class="single_select_page">
						...
						<td class="forminp">
							<?php echo str_replace( ' id=', " data-placeholder='" . esc_attr__( 'Select a page&hellip;', 'nepali-calendar' ) . "' style='" . $value['css'] . "' class='" . $value['class'] . "' id=", wp_dropdown_pages( $args ) ); ?> <?php echo $description; ?>
						</td>
					</tr><?php
					break;
				...
				case 'multi_select_countries' :
					...
					?><tr valign="top">
						...
						<td class="forminp">
							<select multiple="multiple" name="<?php echo esc_attr( $value['id'] ); ?>[]" style="width:350px" data-placeholder="<?php esc_attr_e( 'Choose countries&hellip;', 'nepali-calendar' ); ?>" aria-label="<?php esc_attr_e( 'Country', 'nepali-calendar' ) ?>" class="wc-enhanced-select">
								<?php
									if ( ! empty( $countries ) ) {
								...
								?>
							</select> <?php echo ( $description ) ? $description : ''; ?> <br /><a class="select_all button" href="#"><?php _e( 'Select all', 'nepali-calendar' ); ?></a> <a class="select_none button" href="#"><?php _e( 'Select none', 'nepali-calendar' ); ?></a>
						</td>
					</tr><?php
					break;
				case 'descriptions':
				...
				default:
					do_action( 'nepali_calendar_admin_field_' . $value['type'], $value );
					break;
			}
		}
	}
...
*@return bool
	...
	public static function save_fields( $options, $data = null ) {
		...
		foreach ( $options as $option ) {
			 ...
			 * @since 1.0.0
			 */
			$value = apply_filters( 'nepali_calendar_admin_settings_sanitize_option', $value, $option, $raw_value );

			/**
			 * Sanitize the value of an option by option name.
			 * @since 1.0.0
			 */
			$value = apply_filters( "nepali_calendar_admin_settings_sanitize_option_$option_name", $value, $option, $raw_value );
			 ...
			 * @deprecated 2.4.0 - doesn't allow manipulation of values!
			 */
			do_action( 'nepali_calendar_update_option', $option );
		}
	...
	public static function check_download_folder_protection() {
		...
		$downloads_url   = $upload_dir['basedir'] . '/nepali_calendar_uploads';
		$download_method = get_option( 'nepali_calendar_file_download_method' );
