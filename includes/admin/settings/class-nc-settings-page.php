<?php
/**
 * Nepali CalendarSettings Page/Tab
 *
 * @author      ThemeEgg
 * @category    Admin
...
if (!class_exists('NC_Settings_Page', false)) :
    ...
    abstract class NC_Settings_Page
    {
        ...
        {
            return apply_filters('nepali_calendar_get_settings_' . $this->id, array());
        }
        ...
        {
            return apply_filters('nepali_calendar_get_sections_' . $this->id, array());
        }
        ...
        public function output_sections()
        {
            ...
            foreach ($sections as $id => $label) {
                echo '<li><a href="' . admin_url('admin.php?page=nepali-calendar&tab=' . $this->id . '&section=' . sanitize_title($id)) . '" class="' . ($current_section == $id ? 'current' : '') . '">' . $label . '</a> ' . (end($array_keys) == $id ? '' : '|') . ' </li>';
            }
